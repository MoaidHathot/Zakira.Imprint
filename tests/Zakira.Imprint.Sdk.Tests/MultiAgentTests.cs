...
public class MultiAgentTests : IDisposable
{
    ...
    public void MergeMcp_TwoAgents_ServerInBothFiles()
    {
        ...
        Assert.NotNull(claudeDoc["servers"]?["sample-server"]);
    }
    ...
    public void MergeMcp_ThreeAgents_ServerInAllFiles()
    {
        ...
        // All should have the server
        foreach (var mcpPath in new[]
        {
            Path.Combine(_projectDir, ".vscode", "mcp.json"),
            Path.Combine(_projectDir, ".claude", "mcp.json"),
            Path.Combine(_projectDir, ".cursor", "mcp.json")
        })
        {
            var doc = JsonNode.Parse(File.ReadAllText(mcpPath))!;
            Assert.NotNull(doc["servers"]?["my-server"]);
        }
    }
    ...
    public void MergeMcp_MultipleFragments_TwoAgents()
    {
        ...
        // Both agents should have both servers
        ...
        Assert.NotNull(cursorDoc["servers"]?["server-a"]);
        Assert.NotNull(cursorDoc["servers"]?["server-b"]);
    }
    ...
    public void MergeMcp_TwoAgents_PreservesExistingUserServers()
    {
        ...
        // Copilot should have both user and managed servers
        ...
        // Claude should only have managed server (no pre-existing)
        var claudeDoc = JsonNode.Parse(File.ReadAllText(Path.Combine(_projectDir, ".claude", "mcp.json")))!;
        Assert.NotNull(claudeDoc["servers"]?["managed-server"]);
        Assert.Null(claudeDoc["servers"]?["my-custom-server"]);
    }
